...
class EdgeKioskPage(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Timer polling API system command
        self._command_poll_timer = QTimer()
        self._command_poll_timer.timeout.connect(self._poll_system_command)
    ...
    def _build_overlay(self):
        ...
        # ── Overlay Standby (Chờ lệnh) ──
        self._standby_overlay = QFrame(self._container)
        self._standby_overlay.setFixedSize(500, 150)
        self._standby_overlay.setStyleSheet("""
            QFrame {
        ...
        """)
        standby_layout = QVBoxLayout(self._standby_overlay)
        self._standby_title = QLabel("⏳ ĐANG CHỜ LỆNH TỪ SERVER...")
        self._standby_title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self._standby_title.setStyleSheet("""
            color: white; font-size: 20px; font-weight: 800;
            letter-spacing: 1px; background: transparent;
        """)
        standby_layout.addWidget(self._standby_title)
        self._standby_overlay.hide()
    ...
    # ─── System Command Polling ───────────────
    def _start_polling(self):
        self._command_poll_timer.start(2000) # Poll every 2s
        self._show_standby(True)

    def _poll_system_command(self):
        try:
            from config import edge_config
            import requests
            url = edge_config.api_url + "/api/system/command"
            resp = requests.get(url, headers={"X-API-Key": edge_config.api_key}, timeout=2)
            if resp.status_code == 200:
                data = resp.json()
                cmd = data.get("command", "STOP")
                
                if cmd == "START" and self._current_command != "START":
                    self._current_command = "START"
                    self._show_standby(False)
                    self.start_processing()
                
                elif cmd == "STOP" and self._current_command != "STOP":
                    self._current_command = "STOP"
                    self._show_standby(True)
                    self.stop_processing()
        except:
            pass # Ignore connection error, keep current state

    def _show_standby(self, show: bool):
        if show:
            self._standby_overlay.move(
                (self._container.width() - self._standby_overlay.width()) // 2,
                self._container.height() // 2 - self._standby_overlay.height() // 2,
            )
            self._standby_overlay.show()
            self._standby_overlay.raise_()
        else:
            self._standby_overlay.hide()
    ...
    def start_processing(self):
        ...
        self._camera_label.setText("Đang khởi động Camera & AI...")
    ...
    def showEvent(self, event):
        ...
        # Bắt đầu nghe ngóng Server thay vì auto chạy Camera
        QTimer.singleShot(100, self._start_polling)
