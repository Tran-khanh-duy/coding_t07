...
class AttendanceWorker(QThread):
    ...
    def run(self):
        ...
        cap.set(cv2.CAP_PROP_BUFFERSIZE, 1) # Áp dụng để giảm trễ cho mọi loại camera
        
        frame_skip = 0
        stats_timer = time.time()
        
        # Dùng một queue cho AI processing background
        import threading
        
        class AIPipelineThread(threading.Thread):
            ...
            def get_latest_results(self):
                with self.lock:
                    return self.latest_results, self.latest_elapsed_ms
            ...
            def run(self):
                while self._running:
                    ...
                    if attendance_service.is_active:
                        ...
                        for event in events:
                            self.worker.attendance_done.emit({
        ...
        ai_thread = AIPipelineThread(self)
        ai_thread.start()
        ...
        while self._running:
            ...
            # Copy latest results from AI thread and draw them on the live frame
            results, elapsed_ms = ai_thread.get_latest_results()
