...
class CaptureWorker(QThread):
    ...
    def run(self):
        ...
        if str(source).startswith(("rtsp", "http")):
...
class EnrollPage(QWidget):
    ...
    def _on_update_student(self):
        ...
        try:
            ...
            if self._captured_frames:
                ...
                enrollment_service.start_capture(student_id=self._current_student_id, photo_count=len(self._captured_frames))
                enrollment_service._capture.frames = self._captured_frames.copy()
                result = enrollment_service.finish_enrollment()

                if result.success:
                    self._set_create_status(f"✅ Cập nhật thành công! Ảnh hợp lệ: {result.photos_valid}/{result.photos_taken}", Colors.GREEN)
                    self._lbl_result.setText(f"✅ Đã cập nhật!\n[{result.student_code}] {result.full_name}\nẢnh hợp lệ: {result.photos_valid}/{result.photos_taken}")
                    self._lbl_result.setStyleSheet(f"font-size: 15px; font-weight: 800; color: {Colors.GREEN}; border: none; background: transparent;")
    ...
    def _finish_enrollment(self):
        ...
        try:
            ...
            self._result_card.show()
            if result.success:
                self._lbl_result.setText(f"🎉 Đăng ký thành công!\n[{result.student_code}] {result.full_name}\nẢnh hợp lệ: {result.photos_valid}/{result.photos_taken} | Chất lượng: {result.avg_det_score*100:.1f}%")
                self._lbl_result.setStyleSheet(f"font-size: 15px; font-weight: 800; color: {Colors.GREEN}; border: none; background: transparent;")
                self._result_card.setStyleSheet(card_style(Colors.GREEN + "40", radius=12))
                self._btn_enroll.setText("✅  Đã hoàn tất")
            else:
                self._lbl_result.setText(f"❌ {result.error_msg}")
                self._lbl_result.setStyleSheet(f"font-size: 15px; font-weight: 800; color: {Colors.RED}; border: none; background: transparent;")
